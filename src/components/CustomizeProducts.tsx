       ...
       { productOptions.map(option => (
           <div className="flex flex-col gap-4" key={option.name}>
                ...
                {option.choices?.map((choice) => {
                    ...
                    const clickHander = disabled ? undefined : () => handleOptionSelect(option.name!, choice.description!);
                    return option.name == 'Color' ? (
                        <li key={choice.description} className="w-8 h-8 rounded-full ring-1 ring-gray-300 cursor-pointer relative" 
                        ...
                        <li key={choice.description} className="ring-1 ring-SHOP text-SHOP rounded-md py-1 px-4 text-sm"  
                            style={{cursor: disabled ? "not-allowd": 'pointer',
